...
def B(S, nSwimmers, Vrecreation, R, Vhealth, C):
    return S * nSwimmers * (Vrecreation - (R * Vhealth / 1000.0)) - C
                ...
                for FPR in FPRs:
                    for SWIMMERS in SWIMMERSs:
                        for C in Cs:
                            ...
                            B_TP = [B(-1, SWIMMERS, Vrecreation, R(E), Vhealth, C) for E in hi_readings]
                            if len(B_TP):
                                B_TP = sum(B_TP) / len(B_TP)
                            else:
                                B_TP = 0.0

                            B_FN = [B(1, SWIMMERS, Vrecreation, R(E), Vhealth, C) for E in hi_readings]
                            if len(B_FN):
                                B_FN = sum(B_FN) / len(B_FN)
                            else:
                                B_FN = 0.0

                            B_FP = [B(-1, SWIMMERS, Vrecreation, R(E), Vhealth, C) for E in lo_readings]
                            if len(B_FP):
                                B_FP = sum(B_FP) / len(B_FP)
                            else:
                                B_FP = 0.0

                            B_TN = [B(1, SWIMMERS, Vrecreation, R(E), Vhealth, C) for E in lo_readings]
                            if len(B_TN):
                                B_TN = sum(B_TN) / len(B_TN)
                            else:
                                B_TN = 0.0

                            u = utility(TP, FN, FP, TN, B_TP, B_FN, B_FP, B_TN)
                            print(u)
                            costs.append(u)
